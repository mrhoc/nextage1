...
##-----------------------------------------------------------------------------------------------------------------##
#
#  【MultiLangForm】PHPメールプログラム　多言語対応版 最終更新日2018/07/27　（文字コードはUTF-8のみ可）
#  対応言語についてはすべては把握しておりません。ただUTF-8のためメジャーな言語は対応していると思われます。
#　改造や改変は自己責任で行ってください。
#	
...
$Referer_check_domain = "nextage1.co.jp";
...
$require = array('お名前','E-mail','確認用E-mail');
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<p class=\"error_messe\">【".$key."】".$mailCheckMesse."</p>\n";
				$empty_flag = 1;
			}
		}
	}
...
<?php }else{ ?>
<h2 class="title"><?php echo $confirmTitle;?></h2>
<p align="center"><?php echo $confirmMesse;?></p>
<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
<dl class="formTable form-list">
<?php echo confirmOutput($_POST);//入力内容を表示?>
</dl>
<p align="center"><input type="hidden" name="mail_set" value="confirm_submit">
<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
<input type="submit" value="　<?php echo $submitBtnMesse;?>　" class="btn btn-primary">
<input type="button" value="<?php echo $returnBtnMesse;?>" onClick="history.back()" class="btn"></p>
</form>
<?php copyright();} ?>
</div><!-- /formWrap -->
<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
...
<!--
    <div class="section contact bg_primary">
      <div class="container center"><a class="btn" href="./contact.html">
          <picture>
            <source srcset="images/cmn_contact_sp.png" media="(max-width: 750px)"/><img src="images/cmn_contact_pc.png" alt="CONTACT"/>
          </picture></a></div>
    </div>
-->
    <footer class="footer">
...
function confirmOutput($arr){
	...
	foreach($arr as $key => $val) {
		...
		$html .= "<div class='form-item'><dt>".underline2space($key)."</dt><dd>".$out;
